# -*- coding: utf-8 -*-
"""
Created on Mon Oct 30 19:28:39 2017
...
PREP=Rule("PREP",Production("在"))
ADV=Rule("ADV",Production("在"))
V=Rule("V",Production("在"),Production("编辑"),Production("学习"))
SURNAME=Rule("SURNAME",Production("柳"))
N=Rule("N",Production("柳"),Production("编辑"),Production("学习"),Production("文件"))
ADJ=Rule("ADJ",Production("小"))

NP = Rule("NP", Production(N),Production(SURNAME,N),Production(N,N),Production(V,N),Production(ADJ,N))
PP = Rule("PP",Production(PREP,NP))
VP = Rule("VP", Production(V, NP))
VP.add(Production(ADV,VP))
VP.add(Production(PP,VP))
S = Rule("S", Production(NP, VP))

for tree in build_trees(parse(S, "小 柳 在 编辑 学习 文件")):
    print("--------------------------")
    tree.print_()
